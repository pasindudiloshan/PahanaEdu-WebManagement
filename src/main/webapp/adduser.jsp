  ...
  <div class="main-content">
    ...
    <div class="table-card">
      ...
      <div style="padding: 24px;">
        <form action="adduser" method="post" enctype="multipart/form-data" id="addUserForm">
...
<script>
  ...
  // Front-end validation for empty fields, weak password, and contact number
  form.addEventListener("submit", function(e) {
    const requiredFields = ["uempid", "name", "email", "pass", "contact", "role", "photo"];
    for (let id of requiredFields) {
      const input = document.getElementById(id);
      if (!input || input.value.trim() === "") {
        e.preventDefault();
        swal("Warning", `Please fill in the ${id} field.`, "warning");
        input.focus();
        return false;
      }
    }
    ...
    if (!/^\d{10}$/.test(contactInput.value)) {
      e.preventDefault();
      swal("Warning", "Phone number should be exactly 10 digits.", "warning");
      contactInput.focus();
      return false;
    }
  });
