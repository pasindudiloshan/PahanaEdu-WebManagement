    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (CustomerDao.getCustomerByAccount(accountNumber) != null) {
                response.sendRedirect("addcustomer.jsp?status=duplicate_account");
                return;
            }

            // 🔹 Check for duplicate email (if provided)
            if (email != null && !email.isEmpty() && CustomerDao.emailExists(email)) {
                response.sendRedirect("addcustomer.jsp?status=duplicate_email");
                return;
            }
