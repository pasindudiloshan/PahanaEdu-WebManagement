...
public class CustomerDao {
    ...
    // Update customer (with/without image)
    ...
    // Check if email already exists (for new insert)
    ...
    // Check if email exists for another customer (for update)
    public static boolean emailExistsForOther(String accountNumber, String email) {
        String sql = "SELECT 1 FROM customers WHERE email = ? AND account_number <> ? LIMIT 1";
        try (Connection conn = DBUtil.getConnection();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
